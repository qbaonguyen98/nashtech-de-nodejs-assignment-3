...
class GarageService {
  ...
  public async addServices(
    ...
    serviceIds: GarageUpdateServicesDto,
    userId: string,
  ): Promise<boolean> {
    const garage = await this.garageRepository.findOne(conditions, {
      useLean: true,
    ...
    if (!garage) throw new HttpException(404, 'Garage not found');

    const services = await this.serviceRepository.find({ _id: { $in: serviceIds.services } }, { throwNotFoundException: true });
    if (_.isEmpty(services)) throw new HttpException(404, 'Services not found');

    const garageServicesData: GarageServiceInterface[] = [];
    ...
    return await this.garageServiceRepository.createMultiple(garageServicesData, userId);
  }
   ...
   * @returns {Boolean}
   */
  public async deleteServices(conditions: MongooseFilterQuery<GarageDocument> = {}, serviceIds: GarageUpdateServicesDto): Promise<boolean> {
    const garage = await this.garageRepository.findOne(conditions, {
      useLean: true,
      throwNotFoundException: true,
    });
    ...
    if (_.isEmpty(services)) return true;
    return await this.garageServiceRepository.removeMultiple(garageServicesData);
  }

...
class ServiceController {
  ...
  public updateById = async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
    try {
      const userId: string = req.user.id;
      const serviceId: string = _.get(req.params, 'serviceId', '');
  ...
  public deleteById = async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const result = await this.serviceService.deleteOne(
        {
          _id: serviceId,
        },
        userId,
      );
      if (result) res.status(200).json({ message: 'Service is deleted' });
    } catch (error) {
      next(error);
    }
  };
